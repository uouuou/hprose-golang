...
func TestDecodeIntArrayPtr(t *testing.T) {
	...
	dec.Decode(&array)
	assert.Nil(t, array) // nil
	dec.Decode(&array)
	assert.Equal(t, [5]int{}, *array) // ""
	array[3] = 3
	assert.Equal(t, [5]int{0, 0, 0, 3, 0}, *array)
	dec.Decode(&array)
	assert.EqualError(t, dec.Error, `hprose/io: can not cast int to [5]int`) // 1
}
...
func TestDecodeCustomIntArray(t *testing.T) {
	...
	assert.Equal(t, [5]Int{}, array) // ""
	dec.Decode(&array)
	assert.EqualError(t, dec.Error, `hprose/io: can not cast int to [5]io_test.Int`) // 1
}
...
func TestDecodeByteArray(t *testing.T) {
	...
	dec.Decode(&array)
	assert.EqualError(t, dec.Error, `hprose/io: can not cast int to [5]uint8`) // 1
}
...
func TestDecodeInterfaceArray(t *testing.T) {
	...
	dec.Decode(&array)
	assert.EqualError(t, dec.Error, `hprose/io: can not cast int to [5]interface {}`) // 1
}
...
func TestDecodeIntIntArray(t *testing.T) {
	...
	dec.Decode(&array)
	assert.EqualError(t, dec.Error, `hprose/io: can not cast int to [2][3]int`) // 1
}

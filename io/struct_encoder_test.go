...
func TestAmbiguousFields(t *testing.T) {
	assert.PanicsWithValue(t, "hprose/io: ambiguous fields with the same name or alias: a", func() {
		type TestStruct struct {
			A int `hprose:"a"`
			B int `json:"a"`
		}
...
func TestInvalidStructName(t *testing.T) {
	assert.PanicsWithValue(t, "hprose/io: invalid UTF-8 in struct name", func() {
		type TestStruct struct {
			A int
			B int
		}
		RegisterName("\xFE", (*TestStruct)(nil))
		sb := &strings.Builder{}
		enc := NewEncoder(sb).Simple(false)
		enc.Encode(TestStruct{})
	})

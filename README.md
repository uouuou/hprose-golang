...
2.4 倍。压测口径：127.0.0.1 回环、一元 Echo、单客户端连接多路复用、两轮取中位数、
Go 1.26（Windows amd64）。

第二轮优化（2026-09，先 profile 再动手）：TCP 主链路每请求开销清零（分配 -21%、
p99 -6.6%），HTTP/2 通道定位并修复平台期根因——单条 HTTP/2 连接内帧是串行刷写的
（每请求两次 Flush 均在连接级写锁下，且无跨流合帧），据此加入默认
`min(GOMAXPROCS, 4)` 条的连接分片，h2c/h2 高并发吞吐提升约 2.5~3 倍。

| 场景             |                     优化前 |                     优化后 | 说明            |
|------------------|---------------------------:|---------------------------:|-----------------|
| h2c 64B × 128    |   15,775 QPS（p50 8.47ms） |   47,143 QPS（p50 2.29ms） | 3.0×，连接分片  |
| h2 TLS 64B × 128 |                 17,382 QPS |                 43,251 QPS | 2.5×            |
| h2c 64B × 16     |                 15,509 QPS |                 36,046 QPS | 2.3×            |
| tcp 64B × 128    | 425,397 QPS（p99 1.094ms） | 440,136 QPS（p99 1.022ms） | 每请求分配 -21% |

连接分片可通过给 `Transport.HTTPClient` 赋自定义 Transport 退回单连接；
`http://` 明文路径不受影响（HTTP/1.1 连接池本身可开多条连接）。
